...
/**
 * A builder pattern class which generates a {@link CommandLineRequest}. An
 * explanation of methods:
 * 
 * <p>
 * <strong>{@link #withEvaluator(CommandLineEvaluator)}</strong><br/>
 * By default, process results are evaluated with a
 * {@link CommandLineEvaluatorExitStatusImpl}, which does nothing more than fail
 * processes which return a non-zero exit code. This behavior can be changed by
 * invoking {@link #withEvaluator(CommandLineEvaluator)} with either an existing
 * or anonymous evaluation function.
 * </p>
 * 
 * <p>
 * <strong>{@link #interruptOnFailure()}</strong><br/>
 * By default, processes are <strong>not</strong> interrupted when a failure is
 * first detected. For example, let us take the request:
 * 
 * <pre>
 * new CommandLineRequestBuilder().build(&quot;echo ERROR; sleep 5; echo END&quot;);
 * </pre>
 * 
 * Even if the {@link #evaluator} detects that "ERROR" indicates a failure, by
 * default the process will not be halted until the command completes. This
 * behavior can be circumvented with:
 * 
 * <pre>
...
public class CommandLineRequestBuilder {
	...
	/**
	 * Indicates that the {@link Process} should be interrupted at the first
	 * indication of a failure identifed by the {@link #evaluator}.
	 */
	 ...
	 * 
	 * @param command Non-null and non-blank command line arguments
	 * @return the request
	 */
	public CommandLineRequest build(String command) {
		List<Argument> arguments = new ArrayList<Argument>();
		for (String rawArgument : command.split(" ")) {
			arguments.add(new Argument(rawArgument));
		}
	 ...
	 * 
	 * @param arguments Non-null and non-empty command line arguments
	 * @return the request
	 */

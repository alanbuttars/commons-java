...
/**
 * Test class for {@link WatchFileYamlClassImplStub}.
 * 
 * @author Alan Buttars
 *
 */
public class WatchFileYamlClassImplStubTest {
	...
	public void setup() throws IOException {
		this.file = WatchTestHelper.getSourceFile("user-yaml");
		this.eventBus = mock(EventBus.class);
	}

	@Test
	public void testWithEventBus() throws IOException {
		ConfigurationYamlImpl<User> config = new WatchFileYamlClassImplStub<>(file, User.class).withEventBus(eventBus);
	...
	public void testWithNullEventBus() throws IOException {
		try {
			new WatchFileYamlClassImplStub<>(file, User.class).withEventBus(null);
		}

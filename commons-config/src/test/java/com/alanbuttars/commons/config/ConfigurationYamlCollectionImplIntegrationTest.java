...
/**
 * Integration test for {@link ConfigurationYamlCollectionImpl}.
 * 
 * @author Alan Buttars
 *
 */
public class ConfigurationYamlCollectionImplIntegrationTest extends ConfigurationIntegrationTest<ConfigurationYamlCollectionImpl<List<User>>, List<User>> {
	...
	@Override
	protected ConfigurationYamlCollectionImpl<List<User>> config(Watch watch, String sourceId) throws IOException {
		return watch.yaml(sourceId).mappedTo(new TypeReference<List<User>>() {
		});
	}
	...
	public void testFileAttributeChanged() throws IOException {
		...
		sleep(2);
		assertEquals(1, config.getValue().size());
		assertNull(config.getValue().get(0));
	}

...
public class DecompressFileStub {

	protected final File source;

	DecompressFileStub(File source) {
		this.source = source;
	}
	 ...
	 */
	public DecompressFileWithStubBzipImpl withBzip() {
		return new DecompressFileWithStubBzipImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#DEFLATE}.
	 */
	public DecompressFileWithStubDeflateImpl withDeflate() {
		return new DecompressFileWithStubDeflateImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#GZIP}.
	 */
	public DecompressFileWithStubGzipImpl withGzip() {
		return new DecompressFileWithStubGzipImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#LZMA}.
	 */
	public DecompressFileWithStubLzmaImpl withLzma() {
		return new DecompressFileWithStubLzmaImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#PACK200}.
	 */
	public DecompressFileWithStubPack200Impl withPack200() {
		return new DecompressFileWithStubPack200Impl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#SNAPPY}.
	 */
	public DecompressFileWithStubSnappyImpl withSnappy() {
		return new DecompressFileWithStubSnappyImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#XZ}.
	 */
	public DecompressFileWithStubXzImpl withXz() {
		return new DecompressFileWithStubXzImpl(source);
	}

	/**
	 * Indicates that the {@link #source} will be decompressed with {@link CompressedFiles#Z}.
	 */
	public DecompressFileWithStubZImpl withZ() {
		return new DecompressFileWithStubZImpl(source);
	}
	 ...
	 */
	public DecompressFileWithStub with(String fileType, //
			final Function<InputStream, CompressedFileInputStream> compressionFunction) {
		verifyNonNull(fileType, "File type must be non-null");
		verify(!fileType.trim().isEmpty(), "File type must be non-empty");
		verifyNonNull(compressionFunction, "Decompression function must be non-null");

		return new DecompressFileWithStub(source, fileType) {

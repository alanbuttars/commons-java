 ...
 * 
 * <pre>
 * Archive.directory(directory).asZip().withEncoding("UTF8").to(file);
 * </pre>
 * 
 * <p>
 * <code>withEncoding()</code> is a builder function supported by {@link ArchiveFileWithStubZipImpl}.
 * 
 * @author Alan Buttars
 *
 */
abstract class ArchiveFileWithStub {

	protected final File source;
	protected final String archiveType;

	ArchiveFileWithStub(File source, String archiveType) {
		this.source = source;
	...
	public void to(File destination) throws IOException {
		Archives.archive(archiveType, source, destination, streamConfigFunction(), streamFunction(), entryConfigFunction(), entryFunction());
	}

 ...
 * 
 * <pre>
 * Compress.file(file).asBzip().withBlockSize(1024).to(file);
 * </pre>
 * 
 * <p>
 * <code>withBlockSize()</code> is a builder function supported by {@link CompressFileAsStubSnappyImpl}.
 * 
 * @author Alan Buttars
 *
 */
abstract class CompressFileWithStub {
	...
	public void to(File destination) throws IOException {
		...
		verify(destination.isFile(), "Destination " + destination.getAbsolutePath() + " must not be a directory");

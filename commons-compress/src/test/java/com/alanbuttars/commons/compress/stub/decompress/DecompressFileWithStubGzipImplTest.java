...
/**
 * Test class for {@link DecompressFileWithStubGzipImpl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressFileWithStubGzipImplTest {

	private File source;
	private File destination;
	private DecompressFileWithStubGzipImpl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressFileWithStubGzipImpl(source));
	}

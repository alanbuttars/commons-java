...
/**
 * Test class for {@link ArchiveFileWithStubJarImpl}.
 * 
 * @author Alan Buttars
 *
 */
public class ArchiveFileWithStubJarImplTest {

	private File directory;
	private File destination;
	private ArchiveFileWithStubJarImpl stub;

	@Before
	public void setup() throws IOException {
		directory = Files.createTempDirectory(getClass().getName()).toFile();
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new ArchiveFileWithStubJarImpl(directory));
	}
	...
	public void testConstructor() {
		...
		assertEquals(ArchiveConfigs.OUTPUT_CONFIG_FUNCTIONS.get(JAR), stub.streamConfigFunction());
		assertNotEquals(ArchiveConfigs.OUTPUT_STREAM_FUNCTIONS.get(JAR), stub.streamFunction());
	}

	@Test
	public void testStreamFunction() throws ArchiveException {
		doReturn(null).when(stub).createArchiveOutputStream(//
		...
		verify(stub).createArchiveOutputStream(//
				any(OutputStream.class), //
				eq("UTF8"), //
				eq(""), //

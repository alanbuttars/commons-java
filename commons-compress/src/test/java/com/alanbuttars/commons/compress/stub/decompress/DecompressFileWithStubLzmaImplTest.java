...
/**
 * Test class for {@link DecompressFileWithStubLzmaImpl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressFileWithStubLzmaImplTest {

	private File source;
	private File destination;
	private DecompressFileWithStubLzmaImpl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressFileWithStubLzmaImpl(source));
	}

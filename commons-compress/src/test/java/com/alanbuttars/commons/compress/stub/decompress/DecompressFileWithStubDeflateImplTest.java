...
/**
 * Test class for {@link DecompressFileWithStubDeflateImpl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressFileWithStubDeflateImplTest {

	private File source;
	private File destination;
	private DecompressFileWithStubDeflateImpl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressFileWithStubDeflateImpl(source));
	}

...
/**
 * Test class for {@link DecompressFileWithStubZImpl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressFileWithStubZImplTest {

	private File source;
	private File destination;
	private DecompressFileWithStubZImpl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressFileWithStubZImpl(source));
	}

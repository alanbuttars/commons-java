...
/**
 * Test class for {@link DecompressCompressedFileWithStubXzImpl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressCompressedFileWithStubXzImplTest {

	private File source;
	private File destination;
	private DecompressCompressedFileWithStubXzImpl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressCompressedFileWithStubXzImpl(source));
	}

...
/**
 * Test class for {@link ExtractFileWithStubDumpImpl}
 * 
 * @author Alan Buttars
 *
 */
public class ExtractFileWithStubDumpImplTest {

	private File archive;
	private ExtractFileWithStubDumpImpl stub;

	@Before
	public void setup() throws IOException {
		archive = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new ExtractFileWithStubDumpImpl(archive));
	}

	@Test
	public void testConstructor() {
		assertEquals(archive, stub.source);
		assertEquals(DUMP, stub.archiveType);
		assertEquals(ArchiveConfigs.INPUT_CONFIG_FUNCTIONS.get(DUMP), stub.streamConfigFunction());
		assertNotEquals(ArchiveConfigs.INPUT_STREAM_FUNCTIONS.get(DUMP), stub.streamFunction());
	}

...
public class CompressedFilesIntegrationAbstractTest {
	...
	protected void testDecompress(String fileType, //
			Function<InputStream, CompressedFileInputStreamConfig> streamConfigFunction) throws IOException {
		try (Reader reader = new FileReader(getConfig(fileType))) {
			...
			for (ArchiveFile testFile : testFiles) {
				...
				CompressedFiles.decompress(fileType, source, destination, streamConfigFunction, null);
	...
	protected void testCompress(String fileType, //
			Function<InputStream, CompressedFileInputStreamConfig> inputStreamConfigFunction, //
			Function<OutputStream, CompressedFileOutputStreamConfig> outputStreamConfigFunction) throws IOException {
		try (Reader reader = new FileReader(getConfig(fileType))) {
			List<ArchiveFile> testFiles = new Gson().fromJson(reader, new TypeToken<List<ArchiveFile>>() {
			}.getType());
			for (ArchiveFile testFile : testFiles) {
				...
				CompressedFiles.decompress(fileType, decompressSource, decompressDestination, inputStreamConfigFunction, null);

				File compressDestination = File.createTempFile(fileType + "__", "." + fileType.toLowerCase());
				CompressedFiles.compress(fileType, decompressDestination, compressDestination, outputStreamConfigFunction, null);
				...
				CompressedFiles.decompress(fileType, compressDestination, decompressSecondDestination, inputStreamConfigFunction, null);

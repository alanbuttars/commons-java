...
/**
 * Test class for {@link DecompressFileWithStubPack200Impl}.
 * 
 * @author Alan Buttars
 *
 */
@RunWith(PowerMockRunner.class)
public class DecompressFileWithStubPack200ImplTest {

	private File source;
	private File destination;
	private DecompressFileWithStubPack200Impl stub;

	@Before
	public void setup() throws IOException {
		source = File.createTempFile(getClass().getName(), ".tmp");
		destination = File.createTempFile(getClass().getName(), ".tmp");
		stub = spy(new DecompressFileWithStubPack200Impl(source));
	}
